...
async def proc_file(message: types.Message, state: FSMContext):
    # Faylni aniqlash (Document yoki Photo)
    file_id = None
    if message.document:
        file_id = message.document.file_id
    elif message.photo:
        file_id = message.photo[-1].file_id # Eng tiniq rasm
...
async def proc_flag(message: types.Message, state: FSMContext):
    ...
    # Bazaga faylni ham qo'shamiz
    await add_new_task(data['title'], data['points'], message.text, data.get('file_id'))
    await state.clear()
    await message.answer(f"✅ <b>Task Created!</b>", parse_mode="HTML")
