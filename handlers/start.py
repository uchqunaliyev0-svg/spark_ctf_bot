...
async def start_cmd(message: types.Message, state: FSMContext):
    ...
    if not user:
        await message.answer("Spark CTF-ga xush kelibsiz.\nNickname kiriting:")
        await state.set_state(Register.nickname)
    else:
        await message.answer(f"Xush kelibsiz, {user[1]}.", reply_markup=get_main_menu())
...
async def set_nickname(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(f"Saqlandi: {nick}", reply_markup=get_main_menu())
